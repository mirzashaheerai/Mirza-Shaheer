...
export const HeroSection = () => {
  ...
  return (
    <section className="w-full bg-white text-zinc-950 px-6 sm:px-8 py-16 md:py-24 relative overflow-hidden flex flex-col items-center justify-center min-h-screen">
      ...
      {/* EDGE PROTECTION: Added max-w-[90vw] to prevent elements touching mobile sides */}
      <div className="w-full max-w-[90vw] sm:max-w-4xl mx-auto text-center space-y-4 z-10 flex flex-col items-center relative">
        
        {/* SCALED TYPOGRAPHY: Shifted from text-5xl to text-4xl on base mobile display widths */}
        <motion.h1 
          ...
          className="text-4xl sm:text-7xl md:text-8xl tracking-tight uppercase leading-[0.85] select-none cursor-default pb-2 syne-title-gradient flex flex-col -space-y-1 sm:-space-y-2 md:-space-y-3"
        >
          <span>Mirza</span>
          <span>Shaheer</span>
        </motion.h1>

        {/* SINGLE ROW COMPACT TYPOGRAPHY - Added clean text wrapping protection */}
        <motion.h2
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.8 }}
          className="text-xs sm:text-lg md:text-xl tracking-[0.05em] uppercase text-black pt-1 pb-4 max-w-full px-2 syne-text sm:whitespace-nowrap"
        >
        ...
        <motion.div 
          ...
          className="flex flex-col sm:flex-row gap-4 w-full justify-center pt-2 max-w-xs sm:max-w-md px-2 sm:px-0"
        >
          <motion.a
            whileHover={{ scale: 1.03, y: -2 }}
            whileTap={{ scale: 0.98 }}
            href="#contact"
            className="flex-1 bg-white rounded-xl py-3.5 font-black tracking-wide text-xs sm:text-sm select-none text-center cursor-pointer block text-black wireframe-3d-glow transition-transform duration-200"
          >
            Initiate Project
          </motion.a>
